...
tasks {
    val copyDockerFiles by registering(Copy::class) {
        ...
        from(file("$buildDir/libs", PathValidation.DIRECTORY))
        include("*")
        exclude("**/*.y*ml")
        into(file("$buildDir/docker", PathValidation.DIRECTORY))
    }

    val imageNameGitLab = "registry.gitlab.com/example/marvel/${name.splitToSequence(' ', '-', '.', '_').joinToString(separator = "") { it.substring(0, 1).toLowerCase() }}"
    val tag             = "latest"
    ...
    register<DockerBuildImage>("buildDocker") {
        ...
        tags   .addAll(setOf("$imageNameGitLab:$tag", "$imageNameGitLab:${version.toString().substringBefore("-SNAPSHOT")}"))
    }
